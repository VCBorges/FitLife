...
def test_to_python_to_return_a_python_dict():
    ...
    field = form_fields.FormField(form_class=MockForm)
    data = {'field1': 'test'}
...
def test_clean_with_required_when_receive_a_empty_dict_to_raise_error():
    ...
    field = form_fields.FormField(form_class=MockForm, required=True)
    data = {}
...
def test_clean_with_required_children_field_to_raise_error_if_data_is_invalid():
    ...
    field = form_fields.FormField(form_class=ChildForm)
    data = {'field1': '   '}
...
def test_is_valid_in_a_parent_form_to_errors_have_child_field1():
    ...
    class ParentForm(BaseForm):
        nested_field = form_fields.FormField(form_class=ChildForm)
...
def test_is_valid_with_child_nested_form_field_have_correct_keys():
    ...
    class ChildForm(BaseForm):
        child_field1 = form_fields.FormField(form_class=ChildChildForm, required=True)

    class ParentForm(BaseForm):
        nested_field = form_fields.FormField(form_class=ChildForm, required=True)
